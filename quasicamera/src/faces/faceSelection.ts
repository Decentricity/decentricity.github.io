import type { DetectedFace, SubjectFaceSelection, SubjectMode } from "../types.js";

export const MAX_PRESERVED_FACE_REFERENCES = 5;

export function selectFacesForSubjectMode(
  faces: DetectedFace[],
  subjectMode: SubjectMode,
  seed: string,
  maxFaces = MAX_PRESERVED_FACE_REFERENCES
...
function zeroFaceSelection(subjectMode: SubjectMode): SubjectFaceSelection {
  const instructions: Record<SubjectMode, string> = {
    landscape: [
      "SUBJECT MODE: LANDSCAPE",
...
function buildSelection(
  subjectMode: SubjectMode,
  strategy: SubjectFaceSelection["strategy"],
...
function uniqueFaces(faces: DetectedFace[]): DetectedFace[] {
...
function rankedSubset(faces: DetectedFace[], seed: string, subjectMode: SubjectMode, maxFaces: number): DetectedFace[] {
  return [...faces]
    .sort((a, b) => scoreFace(b, seed, subjectMode) - scoreFace(a, seed, subjectMode))
    .slice(0, Math.max(0, maxFaces));
}

function weightedDeterministicFace(faces: DetectedFace[], seed: string, subjectMode: SubjectMode): DetectedFace {
...
export function scoreFace(face: DetectedFace, seed: string, subjectMode = "single-person"): number {
