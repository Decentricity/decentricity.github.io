...
export function renderReadout(container, context) {
    ...
    const rows = [
        ...
        ...optionalRows([
            ...
            ["Detector", objectMetrics?.detector],
            ["Classifier", objectMetrics?.classifier ?? undefined],
            ["Backend", objectMetrics?.backend?.toUpperCase()],
            ["Model status", objectMetrics?.modelStatus?.toUpperCase()],
            ["Inference time", objectMetrics ? `${objectMetrics.totalObjectAnalysisMs}ms` : undefined],
            ["Objects detected", objectMetrics ? String(objectMetrics.detectedCount) : undefined],
            ["Relationships derived", context.conCamera?.objectRelationships ? String(context.conCamera.objectRelationships.length) : undefined]
        ]),
