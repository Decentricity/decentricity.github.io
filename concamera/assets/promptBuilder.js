...
import { evLabel, flashLabel, focalDistanceEquivalentMm, focalDistancePromptLabel, focusStyleLabel, groundingModeLabel, sanitizeManualSettings, subjectModeLabel } from "./context/manualSettings.js";
...
function objectPreservationRules(analysis, groundingMode) {
...
function objectStructuredDataLine(analysis, byId) {
...
function sourceReferenceIntro(groundingMode) {
...
function groundingModeRules(groundingMode) {
    if (groundingMode === "free") {
        return [
            "GROUNDING MODE",
            "GROUNDING: FREE",
...
function viewpointPreservationLine(groundingMode) {
    if (groundingMode === "free") {
...
export class PromptBuilder {
    ...
    build(context, faceSelection, objectAnalysis) {
        ...
        const lines = [
            "CONCAMERA IMAGE TRANSFORMATION",
            ...
            "PRIORITY 5 -- CAMERA GEOMETRY: source viewpoint, heading, pitch, roll, framing, focal distance, and virtual lens.",
            ...
            value("Detected source faces", context.conCamera?.detectedFaceCount),
            value("Selected source faces", context.conCamera?.selectedFaceCount),
            value("Subject mapping strategy", context.conCamera?.subjectMappingStrategy),
            value("Face analysis provider", context.conCamera?.faceAnalysisProvider),
            value("Generation grounding", groundingModeLabel(settings.groundingMode)),
            value("Source image attached", context.conCamera?.sourceImageAttached === undefined ? undefined : context.conCamera.sourceImageAttached ? "YES" : "NO"),
            value("Recognized non-human objects", context.conCamera?.recognizedObjects?.map((object) => object.normalizedLabel).join(", ")),
            value("Recognized object relationships", context.conCamera?.objectRelationships?.map((relationship) => `${relationship.subject} ${relationship.predicate} ${relationship.object}`).join("; ")),
            value("Object analysis provider", context.conCamera?.objectAnalysisProvider),
...
function lensPromptProfile(value, fallbackMm) {
...
function focalDistanceRules(lens) {
    return [
        ...
        "Focal distance must not change selected face likeness, object semantics, object relationships, time, weather, location, subject mode, or exposure compensation."
    ];
}
...
function manualInteractionRules(context, lens) {
