...
export interface OverlayRenderInput {
  source: SourcePhotoReference;
  analysis: ObjectAnalysis;
  settings: ConCameraSettings;
  timestamp: string;
}

export interface RenderedConCameraFrame {
  imageDataUrl: string;
  sceneSummary: string;
  renderVersion: string;
  overlayRenderMs: number;
  renderedObjects: RecognizedObject[];
  renderedRelationships: ObjectRelationship[];
}

export interface OverlayRendererDependencies {
...
export class SemanticOverlayRenderer {
  constructor(private readonly dependencies: OverlayRendererDependencies = browserOverlayDependencies()) {}

  async render(input: OverlayRenderInput): Promise<RenderedConCameraFrame> {
...
export function selectOverlayObjects(analysis: ObjectAnalysis, settings: ConCameraSettings = DEFAULT_MANUAL_SETTINGS): RecognizedObject[] {
  ...
  const relationshipCritical = new Set<string>();
...
export function selectOverlayRelationships(
  relationships: ObjectRelationship[],
  objects: RecognizedObject[],
  settings: ConCameraSettings = DEFAULT_MANUAL_SETTINGS
): ObjectRelationship[] {
  const normalized = sanitizeManualSettings(settings);
...
export function affordanceFor(label: string): string[] {
...
export function risksFor(object: RecognizedObject): string[] {
...
function labelForObject(object: RecognizedObject, mode: AnalysisMode, density: ConCameraSettings["overlayDensity"]): string | null {
...
function drawHeader(context: CanvasRenderingContext2D, width: number, timestamp: string, settings: ConCameraSettings): void {
...
function drawUnavailable(context: CanvasRenderingContext2D, width: number, height: number): void {
  context.save();
...
function drawCornerBox(context: CanvasRenderingContext2D, box: PixelBox, ratio: number): void {
...
function drawFullBox(context: CanvasRenderingContext2D, box: PixelBox): void {
  context.save();
  context.strokeStyle = "#67fff0";
  context.shadowColor = "rgba(103, 255, 240, 0.45)";
  context.shadowBlur = 5;
...
function drawRelationshipSummary(
  context: CanvasRenderingContext2D,
  relationships: ObjectRelationship[],
  objects: RecognizedObject[],
  width: number,
...
function drawLabel(context: CanvasRenderingContext2D, text: string, x: number, y: number, width: number): void {
  const paddingX = 7;
  const paddingY = 4;
  const metrics = context.measureText(text);
...
function labelHeight(context: CanvasRenderingContext2D, text: string): number {
  const metrics = context.measureText(text);
  return metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent || 16;
}

function relationshipLabel(subject: RecognizedObject, relationship: ObjectRelationship, object: RecognizedObject): string {
...
function relationshipSummary(objects: RecognizedObject[], relationships: ObjectRelationship[]): string {
...
function foregroundCentral(box: FaceBoundingBox): boolean {
  const centerX = box.x + box.width / 2;
  return centerX >= 0.25 && centerX <= 0.75 && box.y + box.height >= 0.58;
}

function outputDimensions(sourceWidth: number, sourceHeight: number): { width: number; height: number } {
...
function scaleBox(box: FaceBoundingBox, width: number, height: number): PixelBox {
  return {
    x: box.x * width,
    y: box.y * height,
...
function center(box: PixelBox): { x: number; y: number } {
  return {
    x: box.x + box.width / 2,
...
function browserOverlayDependencies(): OverlayRendererDependencies {
  return {
    loadImage,
...
export function persistedRelationshipText(relationships: PersistedObjectRelationship[]): string {
