...
export interface PoseSimulatorConfig {
  enabled: boolean;
  pose: CameraPose;
}
...
export function emptyCameraPose(capturedAt = Date.now()): CameraPose {
  return {
    azimuthDeg: null,
    pitchDeg: null,
    rollDeg: null,
...
export function currentScreenOrientationDeg(): number {
...
export function normalizeSignedScreenAngle(value: number): number {
  const normalized = normalizeDeg(value);
...
export function lerpAngleDeg(from: number | null, to: number | null, alpha: number): number | null {
  if (to === null) {
    return from;
  }
...
export function cameraPoseFromDeviceOrientation(sample: OrientationSample): CameraPose {
...
export function smoothCameraPose(previous: CameraPose, next: CameraPose, alpha = 0.24): CameraPose {
  return {
...
export function cameraPoseWithFreshness(pose: CameraPose, now = Date.now()): CameraPose {
  return {
    ...pose,
...
export function freezeCameraPose(pose: CameraPose, capturedAt = Date.now()): CameraPose {
  return {
...
export function aimLabelForPitch(pitchDeg: number | null): string {
  if (pitchDeg === null) {
    return "Unknown";
  }
...
export function frameLabelForScreen(screenOrientationDeg: number): "Portrait" | "Landscape" {
  return normalizeSignedScreenAngle(screenOrientationDeg) === 0 || normalizeSignedScreenAngle(screenOrientationDeg) === 180
    ? "Portrait"
    : "Landscape";
}

export function directionLabelForAzimuth(azimuthDeg: number | null): string {
  if (azimuthDeg === null) {
    return "unknown";
  }
...
export function parseDebugPose(search: string, now = Date.now()): PoseSimulatorConfig {
  ...
  const defaults: [number, number, number, number] = [237, 38.4, -7.2, 0];
  const values = raw === "1"
    ? defaults
...
function confidenceForPose(
  azimuthDeg: number | null,
  pitchDeg: number | null,
  rollDeg: number | null,
  capturedAt: number,
  now: number
): CameraPose["confidence"] {
...
function orientationMatrix(alphaDeg: number, betaDeg: number, gammaDeg: number): Mat3 {
...
function imageUpVectorForScreen(screenOrientationDeg: number): Vec3 {
...
function rollFromWorldVectors(cameraForward: Vec3, imageUpWorld: Vec3): number | null {
...
function mulMatVec(matrix: Mat3, vector: Vec3): Vec3 {
  return [
...
function normalizeRequired(vector: Vec3): Vec3 {
...
function roundPose(value: number | null): number | null {
