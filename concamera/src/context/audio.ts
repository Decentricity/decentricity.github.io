import type { AmbientAudioFeatures } from "../types.js";
...
export class AmbientAudioSensor {
  ...
  private raf = 0;
  private features: AmbientAudioFeatures = {
    status: "pending",
    descriptor: "Unknown"
  };
  ...
  private tick = (): void => {
    ...
    const speechProbability = clamp(
      spectrum.midEnergy * 1.35 - spectrum.bassEnergy * 0.22 - spectrum.trebleEnergy * 0.2 + rms * 0.8
    );
  ...
  private spectrum(freqData: Uint8Array, sampleRate: number): {
    centroidHz: number;
    bassEnergy: number;
    midEnergy: number;
    trebleEnergy: number;
  ...
  private describe(
    ...
    spectrum: { bassEnergy: number; midEnergy: number; trebleEnergy: number }
  ): AmbientAudioFeatures["descriptor"] {
    if (rms < 0.012) {
      return "Silence";
    }
