...
export interface LocalDetector {
  detect(input: CanvasImageSource): Promise<LocalDetection[]>;
}

export interface LocalClassifier {
  classify(input: CanvasImageSource): Promise<LocalClassification[]>;
}
...
interface RuntimeModels {
  ...
  tf?: TensorFlowRuntime | undefined;
  detectorName: string;
...
export interface LocalCnnObjectAnalyzerOptions {
  detector?: LocalDetector | undefined;
  classifier?: LocalClassifier | null | undefined;
  runtimeLoader?: (() => Promise<RuntimeModels>) | undefined;
  maxDimension?: number | undefined;
  minConfidence?: number | undefined;
  modelLoadTimeoutMs?: number | undefined;
  detectionTimeoutMs?: number | undefined;
  now?: (() => number) | undefined;
  createAnalysisInput?: ((source: SourcePhotoReference, maxDimension: number) => Promise<AnalysisInput>) | undefined;
  createCropInput?: ((input: AnalysisInput, box: FaceBoundingBox) => CanvasImageSource | null) | undefined;
}

interface CandidateObject extends RecognizedObject {
  detectorLabel: string;
  classifierLabels: string[];
}

export class LocalCnnObjectAnalyzer implements ObjectAnalyzer {
  private runtimePromise: Promise<RuntimeModels> | null = null;
  private modelStatus: ObjectAnalysisMetrics["modelStatus"] = "loading";

  constructor(private readonly options: LocalCnnObjectAnalyzerOptions = {}) {}
  ...
  async analyze(source: SourcePhotoReference): Promise<ObjectAnalysis> {
    ...
    try {
      ...
      const detections = await withTimeout(
        scoped(runtime.tf, () => runtime.detector.detect(input?.image as CanvasImageSource)),
        this.options.detectionTimeoutMs ?? DETECTION_TIMEOUT_MS,
  ...
  private async loadRuntime(): Promise<RuntimeModels> {
  ...
  private async classifyCrop(input: AnalysisInput, box: FaceBoundingBox, runtime: RuntimeModels): Promise<string[]> {
    ...
    try {
      ...
      return classifications
        ...
        .filter((label): label is string => typeof label === "string" && !isHumanObjectLabel(label));
    } catch {
      return [];
    }
  }
...
function orderObjectsForPrompt(objects: RecognizedObject[], relationships: ObjectRelationship[]): RecognizedObject[] {
...
async function injectedRuntime(detector: LocalDetector, classifier: LocalClassifier | null, now: () => number): Promise<RuntimeModels> {
  const start = now();
  return {
    detector,
    classifier,
...
async function loadBrowserRuntime(now: () => number = Date.now): Promise<RuntimeModels> {
  const startedAt = now();
  const tf = await importExternal<TensorFlowRuntime>(TFJS_URL);
  if (tf.setBackend) {
  ...
  const [cocoSsd, mobilenet] = await Promise.all([
    importExternal<{ load(options?: Record<string, unknown>): Promise<LocalDetector> }>(COCO_SSD_URL),
    importExternal<{ load(options?: Record<string, unknown>): Promise<LocalClassifier> }>(MOBILENET_URL)
  ]);
...
function normalizeDetectorBox(bbox: LocalDetection["bbox"], width: number, height: number): FaceBoundingBox | null {
...
function fuseDetectionLabel(detectorLabel: string, classifierLabels: string[]): string {
...
function normalizedAttribute(value: string): string | null {
...
export function deriveRelationships(objects: RecognizedObject[]): ObjectRelationship[] {
...
function relationshipFor(subject: RecognizedObject, object: RecognizedObject): ObjectRelationshipPredicate | null {
...
function isOnTopOf(subject: FaceBoundingBox, object: FaceBoundingBox): boolean {
...
function isNextTo(a: FaceBoundingBox, b: FaceBoundingBox): boolean {
...
function isAttachedTo(a: FaceBoundingBox, b: FaceBoundingBox): boolean {
  const iou = intersectionOverUnion(a, b);
  return iou > 0.08 && !containsBox(a, b) && !containsBox(b, a);
}

export function intersectionOverUnion(a: FaceBoundingBox, b: FaceBoundingBox): number {
...
export function horizontalOverlap(a: FaceBoundingBox, b: FaceBoundingBox): number {
  return Math.max(0, Math.min(a.x + a.width, b.x + b.width) - Math.max(a.x, b.x));
}

export function verticalOverlap(a: FaceBoundingBox, b: FaceBoundingBox): number {
  return Math.max(0, Math.min(a.y + a.height, b.y + b.height) - Math.max(a.y, b.y));
}

export function containsBox(outer: FaceBoundingBox, inner: FaceBoundingBox): boolean {
...
function relationshipConfidence(a: FaceBoundingBox, b: FaceBoundingBox, predicate: ObjectRelationshipPredicate): number {
  if (predicate === "inside") {
    return 0.82;
  }
...
function inversePredicate(predicate: ObjectRelationshipPredicate): ObjectRelationshipPredicate | null {
...
async function createAnalysisInput(source: SourcePhotoReference, maxDimension: number): Promise<AnalysisInput> {
...
function createCropInput(input: AnalysisInput, box: FaceBoundingBox): CanvasImageSource | null {
...
function buildMetrics(metrics: ObjectAnalysisMetrics): ObjectAnalysisMetrics {
  return {
    ...metrics,
...
async function scoped<T>(tf: TensorFlowRuntime | undefined, fn: () => Promise<T>): Promise<T> {
...
function centerY(box: FaceBoundingBox): number {
