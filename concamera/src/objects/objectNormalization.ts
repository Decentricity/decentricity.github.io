...
const CATEGORIES: readonly ObjectCategory[] = [
  "animal",
  "toy",
  "vehicle",
  "furniture",
...
export function isHumanObjectLabel(label: string): boolean {
  return HUMAN_LABEL_RE.test(label);
}

export function normalizeObjectLabel(label: string): string | null {
...
export function normalizeObjectCategory(label: string, category?: unknown): ObjectCategory {
  const candidate = stringValue(category);
  if (candidate && CATEGORIES.includes(candidate as ObjectCategory)) {
    return candidate as ObjectCategory;
  }
...
export function normalizePredicate(predicate: string): ObjectRelationshipPredicate | null {
  ...
  return SUPPORTED_OBJECT_PREDICATES.includes(text as ObjectRelationshipPredicate)
    ? text as ObjectRelationshipPredicate
    : null;
}

function normalizedBoundingBox(value: unknown): RecognizedObject["boundingBox"] {
...
function normalizeObject(raw: RawObject, fallbackIndex: number): RecognizedObject | null {
...
function relationshipKey(relationship: ObjectRelationship): string {
  return `${relationship.subjectObjectId}|${relationship.predicate}|${relationship.objectObjectId}`;
}

export function validateObjectAnalysis(raw: unknown, provider = "normalized-object-analysis"): ObjectAnalysis {
  ...
  const objectByOriginalId = new Map<string, RecognizedObject>();
  const objectByNormalizedLabel = new Map<string, RecognizedObject>();
  const omittedObjects: Array<{ label: string; normalizedLabel: string; reason: string }> = [];

  rawObjects.forEach((entry, index) => {
    const objectRecord = asRecord(entry) as RawObject | null;
    if (!objectRecord) {
  ...
  rawRelationships.forEach((entry) => {
    const relationshipRecord = asRecord(entry) as RawRelationship | null;
    ...
    const normalizedRelationship: ObjectRelationship = {
  ...
  const relationshipCriticalIds = new Set<string>();
...
export function toPersistedObjectMetadata(analysis: ObjectAnalysis): {
  recognizedObjects: PersistedRecognizedObject[];
  objectRelationships: PersistedObjectRelationship[];
  warnings: string[];
  omittedObjects?: Array<{ label: string; normalizedLabel: string; reason: string }> | undefined;
} {
