...
test("person mode selects exactly one source face and leaves unselected faces available only as non-identities", () => {
  ...
  assert.equal(selection.selectedFaceCount, 1);
  assert.match(selection.promptInstruction, /Use only the selected reference person as the principal recognizable real person/);
  assert.match(selection.promptInstruction, /Do not preserve the other source faces/);
  assert.match(selection.promptInstruction, /Any incidental background people must be synthetic/);
});
