...
test("manifest is installable under the GitHub Pages subdirectory", async () => {
  assert.equal(manifest.name, "ConCamera");
  assert.equal(manifest.short_name, "ConCam");
...
test("service worker caches the app shell and local CNN model assets without caching API traffic", () => {
  assert.match(sw, /con-camera-static-v1/);
  assert.match(sw, /con-camera-models-v1/);
  ...
  assert.match(sw, /storage\.googleapis\.com/);
