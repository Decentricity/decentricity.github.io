export class LocalPrototypeProvider {
    id = "local-context-imaginer";
    ...
    applyDepthEffect(context, subjectMode) {
        const temp = document.createElement("canvas");
        temp.width = 1024;
        temp.height = 1024;
        const tempContext = temp.getContext("2d");
    ...
    drawSubjectMode(context, random, subjectMode, pitch, isNight) {
        if (subjectMode === "landscape") {
            return;
        }
    ...
    drawPerson(context, x, y, scale, color) {
        context.save();
    ...
    applyFlash(context, subjectMode, enabled, pronounced) {
        if (!enabled) {
            return;
        }
    ...
    addWeatherAndFilm(context, random, rain, audioDescriptor, iso) {
