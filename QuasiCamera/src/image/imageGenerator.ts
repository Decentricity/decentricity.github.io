...
declare global {
  interface Window {
    QUASICAMERA_CONFIG?: AntiCameraRuntimeConfig;
  }
}
...
function readRuntimeConfig(): AntiCameraRuntimeConfig {
  const fromWindow = window.QUASICAMERA_CONFIG || {};
  const storedHeaders = safeStorageGet("quasicamera.endpoint.headers");
  const storedProvider = safeStorageGet("quasicamera.provider");

  return {
    ...fromWindow,
    provider: validProvider(storedProvider) || fromWindow.provider,
    openaiApiKey: safeStorageGet("quasicamera.openai.key") || fromWindow.openaiApiKey,
    openaiModel: safeStorageGet("quasicamera.openai.model") || fromWindow.openaiModel,
    endpointUrl: safeStorageGet("quasicamera.endpoint.url") || fromWindow.endpointUrl,
